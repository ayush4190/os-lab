  ...
  int main()
{
  ...
  int n, ari[10], bur[10], total = 0, i, j, small, temp, procs[100], k, waiting[10]={0}, finish[10];
  ...
  {
    // finish[i]+1 in all 3 lines
    printf("\n PROCESS %d:-FINISH TIME==> %d TURNAROUND TIME==>%d WAITING TIME==>%d\n", i + 1, finish[i] + 1, (finish[i] - ari[i]) + 1, (((finish[i] + 1) - waiting[i]) - bur[i]));
